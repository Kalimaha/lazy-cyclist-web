...
class TestDirectionsParser extends FunSpec {
  ...
  describe(".toRoutes") {
    describe("when the JSON is valid") {
      ...
      it("converts a JSON into a Route") { assert(toRoutes(json) == Right(List(route))) }
    }
    ...
    describe("when the JSON is NOT valid") {
      ...
      it("returns an empty list") { assert(toRoutes(json) == Right(List())) }
    }

    describe("when the JSON is empty") {
      val json = """"{}"""

      it("returns an empty list") { assert(toRoutes(json) == Left("unexpected eof")) }
    }
  }
